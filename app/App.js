...
function SaveModal({ visible, onClose, onSave }) {
  ...
  const [ogp, setOgp] = useState(null); // SNS以外のURLから取得したページ情報（タイトル/写真）
  const [ogpLoading, setOgpLoading] = useState(false);

  const sns = parseSnsLink(link);            // SNSリンクを認識（X/Instagram/YouTube など）
  const previewUri = image || (sns && sns.thumbnail) || (ogp && ogp.image); // 写真未選択でもYouTube/リンクのサムネを表示

  // 熱量を変えると「思い出す（通知）」の既定が出し分けされる
  function chooseHeat(h) { setHeat(h); setReminder({ remind: defaultRemindForHeat(h) }); }

  // SNS以外のURLを貼ったら、ページ情報（タイトル/写真）を自動取得する。
  // タイトルが空欄の時だけ自動入力（手入力中のものは上書きしない）。
  useEffect(() => {
    setOgp(null);
    setOgpLoading(false);
    ...
    const timer = setTimeout(async () => {
      ...
      if (cancelled) return;
      setOgpLoading(false);
      const cleaned = cleanTitle(result.title, url, '');
      if (!cleaned && !result.image) return;
      setOgp({ title: cleaned, image: result.image });
      if (cleaned) setTitle((prev) => (prev.trim() ? prev : cleaned));
    }, 600);
  ...
  function resetForm() { setTitle(''); setCategory('eat'); setDue('none'); setImage(null); setCoords(null); setWithWho(null); setLink(''); setHeat(2); setReminder({ remind: '3days' }); setOgp(null); }
  function handleSave() {
      ...
      <KeyboardAvoidingView style={s.modalBackdrop} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <View style={s.sheet}>
          ...
          <ScrollView keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
            ...
            {!sns && !ogpLoading && ogp && (ogp.title || ogp.image) && (
              <View style={s.snsDetected}>
                <Ionicons name="link" size={15} color={t.accent} />
                <Text style={s.snsDetectedText}>リンク先の情報を取得しました{ogp.title ? `（${ogp.title}）` : ''}</Text>
              </View>
            )}
