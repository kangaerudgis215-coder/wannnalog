...
export default function App() {
  ...
  async function markVisionAchieved(id) {
    ...
    await persistVision(visionSlots.map((x) => (x.id === id ? { ...x, ...achieveVisionPatch(achievedAt) } : x)));
    setCeleb(buildAchievedCeleb(v, achievedAt));
  }
  // 「叶った」から戻す（達成日を消す）。
